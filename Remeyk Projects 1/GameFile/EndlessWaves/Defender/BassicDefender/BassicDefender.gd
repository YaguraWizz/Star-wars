...
func set_number_of_shots(_Asteroid) -> void:
	Number_of_shots=stepify(_Asteroid.HP_comet/get_Damage()*2,1)
...
func intercept_point(ca) -> Vector2: #вычисление точки пересечения траекторий астероида и пули (примерное)
	return ca.global_position + Vector2(global_position.distance_to(ca.global_position)/2/get_Speed()*ca.SPEED,
